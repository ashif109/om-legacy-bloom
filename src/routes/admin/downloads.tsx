...
import { getDownloads, addDownload, updateDownload, deleteDownload } from "@/lib/api";
import Cookies from "js-cookie";
import { Plus, Loader2, X, Edit, Download } from "lucide-react";
...
function AdminDownloads() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await updateDownload({ data: { token, id: editingId, item: formData } });
      } else {
        await addDownload({ data: { token, item: formData } });
      }
      closeModal();
      fetchItems();
    } catch (e: any) {
      alert(e.message || "Error saving downloads.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (item: any) => {
    if (!confirm("Are you sure you want to delete this downloads?")) return;
    try {
      await deleteDownload({ data: { token, id: item._id } });
      fetchItems();
    } catch (e) {
  ...
  const columns: ColumnDef<any>[] = [{ header: "Title", accessorKey: "title" }, { header: "Category", accessorKey: "category" }];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-display text-gold-gradient">Downloads</h1>
          <p className="text-gray-400">Manage your downloads.</p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm custom-scrollbar overflow-y-auto">
          <div className="bg-[color:var(--card)] w-full max-w-md rounded-2xl gold-border p-6 shadow-2xl my-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-display text-cream">{editingId ? "Edit" : "Add"} Download</h2>
