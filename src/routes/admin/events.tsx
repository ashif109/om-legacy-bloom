...
import { Plus, Loader2, X, Edit, Calendar } from "lucide-react";
...
function AdminEvents() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await updateEvent({ data: { token, id: editingId, item: formData } });
      } else {
        await addEvent({ data: { token, item: formData } });
      }
      closeModal();
      fetchItems();
    } catch (e: any) {
      alert(e.message || "Error saving events.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (item: any) => {
    if (!confirm("Are you sure you want to delete this events?")) return;
    try {
      await deleteEvent({ data: { token, id: item._id } });
      fetchItems();
    } catch (e) {
  ...
  const columns: ColumnDef<any>[] = [{ header: "Title", accessorKey: "title" }, { header: "Date", accessorKey: "date" }, { header: "Status", accessorKey: "status" }];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-display text-gold-gradient">Events</h1>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm custom-scrollbar overflow-y-auto">
          <div className="bg-[color:var(--card)] w-full max-w-md rounded-2xl gold-border p-6 shadow-2xl my-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-display text-cream">{editingId ? "Edit" : "Add"} Event</h2>
