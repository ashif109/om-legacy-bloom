...
import { getPublications, addPublication, updatePublication, deletePublication } from "@/lib/api";
import Cookies from "js-cookie";
import { Plus, Loader2, X, Edit, BookOpen } from "lucide-react";
...
function AdminPublications() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await updatePublication({ data: { token, id: editingId, item: formData } });
      } else {
        await addPublication({ data: { token, item: formData } });
      }
      closeModal();
      fetchItems();
    } catch (e: any) {
      alert(e.message || "Error saving publications.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (item: any) => {
    if (!confirm("Are you sure you want to delete this publications?")) return;
    try {
      await deletePublication({ data: { token, id: item._id } });
      fetchItems();
    } catch (e) {
  ...
  const columns: ColumnDef<any>[] = [{ header: "Title", accessorKey: "title" }, { header: "Author", accessorKey: "author" }, { header: "Type", accessorKey: "type" }];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-display text-gold-gradient">Publications</h1>
          <p className="text-gray-400">Manage your publications.</p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm custom-scrollbar overflow-y-auto">
          <div className="bg-[color:var(--card)] w-full max-w-md rounded-2xl gold-border p-6 shadow-2xl my-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-display text-cream">{editingId ? "Edit" : "Add"} Publication</h2>
