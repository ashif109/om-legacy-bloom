...
import { getSeminars, addSeminar, updateSeminar, deleteSeminar } from "@/lib/api";
import Cookies from "js-cookie";
import { Plus, Loader2, X, Edit, Video } from "lucide-react";
...
function AdminSeminars() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await updateSeminar({ data: { token, id: editingId, item: formData } });
      } else {
        await addSeminar({ data: { token, item: formData } });
      }
      closeModal();
      fetchItems();
    } catch (e: any) {
      alert(e.message || "Error saving seminars.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (item: any) => {
    if (!confirm("Are you sure you want to delete this seminars?")) return;
    try {
      await deleteSeminar({ data: { token, id: item._id } });
      fetchItems();
    } catch (e) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-display text-gold-gradient">Seminars</h1>
          <p className="text-gray-400">Manage your seminars.</p>
        </div>
        <button onClick={() => setIsModalOpen(true)} className="btn-gold px-4 py-2 rounded-lg flex items-center gap-2 font-medium">
          <Plus size={18} /> Add Seminar
        </button>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm custom-scrollbar overflow-y-auto">
          <div className="bg-[color:var(--card)] w-full max-w-md rounded-2xl gold-border p-6 shadow-2xl my-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-display text-cream">{editingId ? "Edit" : "Add"} Seminar</h2>
