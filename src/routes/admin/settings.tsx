...
import { getSettingss, addSettings, updateSettings, deleteSettings } from "@/lib/api";
import Cookies from "js-cookie";
import { Plus, Loader2, X, Edit, Settings } from "lucide-react";
...
function AdminSettings() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await updateSettings({ data: { token, id: editingId, item: formData } });
      } else {
        await addSettings({ data: { token, item: formData } });
      }
      closeModal();
      fetchItems();
    } catch (e: any) {
  ...
  const handleDelete = async (item: any) => {
    if (!confirm("Are you sure you want to delete this settings?")) return;
    try {
      await deleteSettings({ data: { token, id: item._id } });
      fetchItems();
    } catch (e) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm custom-scrollbar overflow-y-auto">
          <div className="bg-[color:var(--card)] w-full max-w-md rounded-2xl gold-border p-6 shadow-2xl my-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-display text-cream">{editingId ? "Edit" : "Add"} Settings</h2>
