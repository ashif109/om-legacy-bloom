    ...
    <PageShell>
      ...
      <section className="mx-auto max-w-6xl px-6 pb-32">
        <div className="relative">
          <div className="pointer-events-none absolute left-1/2 top-0 h-full w-px -translate-x-1/2 bg-gradient-to-b from-transparent via-gold/50 to-transparent" />
            ...
            {journey.map((j, i) => (
              ...
              >
                <div className={`${i % 2 ? "md:order-2 md:text-left md:pl-12" : "md:text-right md:pr-12"}`}>
                  <div className="text-xs uppercase tracking-[0.3em] text-gold">{j.year}</div>
                ...
                <div className={`${i % 2 ? "md:order-1 md:pr-12" : "md:pl-12"}`}>
                  <div className="glass-card rounded-2xl p-6 text-sm text-foreground">
                    <div className="font-devanagari text-2xl text-gold">॥ {j.year} ॥</div>
                ...
                <span className="pointer-events-none absolute left-1/2 top-4 hidden h-4 w-4 -translate-x-1/2 rounded-full border-2 border-gold bg-background md:block shadow-[0_0_10px_rgba(196,169,98,0.5)]" />
              </motion.div>
            ))}
          </div>
        </div>
